...
bool readJumpButtonPressed() {
  bool currentReading = digitalRead(JUMP_BUTTON_PIN);
  ...
  if (millis() - lastJumpDebounceTime > BUTTON_DEBOUNCE_DELAY) {
...
void readCrouchButton() {
  bool crouchButtonPressed = (digitalRead(CROUCH_BUTTON_PIN) == LOW);
...
void updateDinosaur(float deltaTime) {
  if (!dinoOnGround) {
    ...
    int standingGroundY = GROUND_Y - DINO_STAND_HEIGHT;
...
void spawnObstacle(bool firstObstacle) {
  ...
  if (firstObstacle) {
    ...
    obstacle.x = SCREEN_WIDTH + random(10, 40);
  }
...
void updateRunAnimation() {
  ...
  if (dinoOnGround && currentTime - lastAnimationTime >= RUN_ANIMATION_INTERVAL) {
...
bool checkCollision() {
  ...
  if (isCrouching && dinoOnGround) {
    ...
    dinoCollisionY = GROUND_Y - DINO_CROUCH_HEIGHT;
  } else {
  ...
  int obstacleHitboxX = (int)obstacle.x + 1;
  int obstacleHitboxY = obstacle.y + 1;
  int obstacleHitboxWidth = obstacle.width - 2;
  int obstacleHitboxHeight = obstacle.height - 2;

  return rectanglesOverlap(
    dinoHitboxX, dinoHitboxY, dinoHitboxWidth, dinoHitboxHeight,
    obstacleHitboxX, obstacleHitboxY, obstacleHitboxWidth, obstacleHitboxHeight
  );
}
...
void drawScore() {
  ...
  unsigned long shownHighScore = highScore > 9999 ? 9999 : highScore;
  unsigned long shownScore = score > 9999 ? 9999 : score;

  char scoreText[15];
  snprintf(scoreText, sizeof(scoreText), "HI %04lu %04lu", shownHighScore, shownScore);
  ...
  int scoreX = SCREEN_WIDTH - textWidth;
  if (scoreX < 0) {
    scoreX = 0;
  }
...
void drawGround() {
  ...
  int groundOffset = ((int)obstacle.x) % 16;

  for (int x = groundOffset; x < SCREEN_WIDTH; x += 16) {
    if (x >= 0) {
      display.drawPixel(x, GROUND_Y + 4, SSD1306_WHITE);
    }
  }
}
...
void drawDinosaur() {
  if (isCrouching && dinoOnGround) {
    drawCrouchingDinosaur(DINO_X, GROUND_Y - DINO_CROUCH_HEIGHT);
  } else {
    drawStandingDinosaur(DINO_X, (int)dinoY);
  }
}

void drawStandingDinosaur(int x, int y) {
  // Head
  display.fillRect(x + 6, y, 8, 7, SSD1306_WHITE);
  // Eye
  display.drawPixel(x + 11, y + 2, SSD1306_BLACK);
  // Mouth
  display.drawPixel(x + 13, y + 5, SSD1306_BLACK);
  // Neck
  display.fillRect(x + 5, y + 5, 5, 6, SSD1306_WHITE);
  // Body
  display.fillRect(x + 3, y + 8, 8, 7, SSD1306_WHITE);

  // Tail
  display.drawLine(x + 3, y + 10, x, y + 7, SSD1306_WHITE);
  display.drawLine(x + 2, y + 11, x, y + 9, SSD1306_WHITE);

  // Arm
  display.drawLine(x + 9, y + 10, x + 13, y + 11, SSD1306_WHITE);

  // Alternate the legs while running
  if (dinoOnGround) {
    if (runFrame) {
      display.drawLine(x + 5, y + 14, x + 3, y + 17, SSD1306_WHITE);
      display.drawLine(x + 9, y + 14, x + 11, y + 17, SSD1306_WHITE);
    } else {
      display.drawLine(x + 5, y + 14, x + 7, y + 17, SSD1306_WHITE);
      display.drawLine(x + 9, y + 14, x + 7, y + 17, SSD1306_WHITE);
    }
  } else {
    // Fixed leg position while jumping
    display.drawLine(x + 5, y + 14, x + 3, y + 16, SSD1306_WHITE);
    display.drawLine(x + 9, y + 14, x + 11, y + 16, SSD1306_WHITE);
  }
}

void drawCrouchingDinosaur(int x, int y) {
  // Longer and lower body
  display.fillRect(x + 3, y + 3, 13, 6, SSD1306_WHITE);
  // Head
  display.fillRect(x + 14, y, 7, 7, SSD1306_WHITE);
  // Eye
  display.drawPixel(x + 18, y + 2, SSD1306_BLACK);
  // Mouth
  display.drawPixel(x + 20, y + 5, SSD1306_BLACK);

  // Tail
  display.drawLine(x + 3, y + 4, x, y + 1, SSD1306_WHITE);
  display.drawLine(x + 3, y + 6, x, y + 4, SSD1306_WHITE);

  // Alternate the crouching leg positions
  if (runFrame) {
    display.drawLine(x + 6, y + 8, x + 4, y + 9, SSD1306_WHITE);
    display.drawLine(x + 12, y + 8, x + 14, y + 9, SSD1306_WHITE);
  } else {
    display.drawLine(x + 7, y + 8, x + 9, y + 9, SSD1306_WHITE);
    display.drawLine(x + 13, y + 8, x + 11, y + 9, SSD1306_WHITE);
  }
}
...
void drawObstacle() {
  ...
  switch (obstacle.type) {
    case SINGLE_CACTUS:
      drawSingleCactus(x, obstacle.y);
      break;
    case DOUBLE_CACTUS:
      drawDoubleCactus(x, obstacle.y);
      break;
    case TRIPLE_CACTUS:
      drawTripleCactus(x, obstacle.y);
      break;
    case LARGE_CACTUS:
      drawLargeCactus(x, obstacle.y);
      break;
    case BIRD:
      drawBird(x, obstacle.y);
      break;
  }
}

void drawSingleCactus(int x, int y) {
  display.fillRect(x + 2, y, 3, 13, SSD1306_WHITE);
  display.fillRect(x, y + 5, 2, 3, SSD1306_WHITE);
  display.fillRect(x + 5, y + 3, 2, 4, SSD1306_WHITE);
}
...
void drawLargeCactus(int x, int y) {
  display.fillRect(x + 4, y, 4, 19, SSD1306_WHITE);
  display.fillRect(x, y + 7, 4, 4, SSD1306_WHITE);
  display.fillRect(x + 7, y + 4, 3, 6, SSD1306_WHITE);
}

void drawBird(int x, int y) {
  display.fillRect(x + 5, y + 3, 7, 4, SSD1306_WHITE);
  display.fillRect(x + 11, y + 1, 4, 4, SSD1306_WHITE);
  display.drawPixel(x + 15, y + 3, SSD1306_WHITE);
  display.drawPixel(x + 13, y + 2, SSD1306_BLACK);

  display.drawLine(x + 5, y + 4, x + 1, y + 2, SSD1306_WHITE);

  if (runFrame) {
    display.drawLine(x + 7, y + 3, x + 3, y, SSD1306_WHITE);
    display.drawLine(x + 8, y + 4, x + 4, y, SSD1306_WHITE);
  } else {
    display.drawLine(x + 7, y + 6, x + 3, y + 8, SSD1306_WHITE);
    display.drawLine(x + 8, y + 6, x + 5, y + 8, SSD1306_WHITE);
  }
}
...
void showGameOverScreen() {
  ...
  snprintf(scoreLine, sizeof(scoreLine), "SCORE: %lu", shownScore);
  snprintf(bestLine, sizeof(bestLine), "BEST: %lu", shownHighScore);
